...
where id=(select class from sms_students where id=(select id from user WHERE id='".$_SESSION['adminUserid']."')";	
		...
		*/
		...
		while( $kelas = mysqli_fetch_assoc($result) ) {		
			...
			*/
			$kelasRows[] = $kelas['id'];
			//$kelasRows[] = ucfirst($kelas['first_name']." ".$kelas['last_name']);
			$kelasRows[] = $kelas['subject'];			
			$kelasRows[] = $kelas['section'];	
			$kelasRows[] = $kelas['teacher'];	
			$kelasRows[] = $kelas['type'];
			/*
			$kelasRows[] = $status;					
			$kelasRows[] = '<button type="button" name="update" id="'.$kelas["id"].'" class="btn btn-warning btn-xs update">Update</button>';
			$kelasRows[] = '<button type="button" name="delete" id="'.$kelas["id"].'" class="btn btn-danger btn-xs delete" >Delete</button>';
			*/
			$kelasData[] = $kelasRows;
		}
		$output = array(
