...
//require('include/config.php');
	...
	//echo "1) FullPath : ".$FullPath."<br>";
	
	//$FileOnly=basename($_SERVER['SCRIPT_FILENAME'],'\\');
	//echo "2) FileOnly : ".$FileOnly."<br>";
	
	//FilePath=rtrim(rtrim($_SERVER['SCRIPT_FILENAME'],$FileOnly),'\\');
	//echo "3) FilePath : ".$FilePath."<br>";
	
	//$HostPath = rtrim(rtrim($FilePath,'HR'),'\\');
	//echo "4) HostPath : ".$HostPath."<br>";
	
	//echo $_SERVER['DOCUMENT_ROOT'];
	$docroot= $HostPath; 	//$docroot="C:\shared\zCMS";	//$path = $_SERVER['DOCUMENT_ROOT'];
	$docroot="C:\shared\SIS1.01";
	$path= "\config.php";
...
class User extends Dbconfig {
	...
	public function login(){		
     ...
     public function staffloginStatus(){
	...
	public function stafflogin(){		
		$errorMessage = '';
		if(!empty($_POST["login"]) && $_POST["staffId"]!=''&& $_POST["staffPass"]!='') {	
			$loginId = $_POST['staffId'];
			$password = $_POST['staffPass'];
			if(isset($_COOKIE["staffPass"]) && $_COOKIE["staffPass"] == $password) {
				$password = $_COOKIE["staffPass"];
			} else {
				$password = md5($password);
			}	
			$sqlQuery = "SELECT * FROM ".$this->userTable." 
			...
			if($isValidLogin){
				if(!empty($_POST["remember"]) && $_POST["remember"] != '') {
					setcookie ("staffId", $loginId, time()+ (10 * 365 * 24 * 60 * 60));  
					setcookie ("staffPass",	$password,	time()+ (10 * 365 * 24 * 60 * 60));
				} else {
					$_COOKIE['staffId' ]='';
					$_COOKIE['staffPass'] = '';
				}
				$userDetails = mysqli_fetch_assoc($resultSet);
				$_SESSION["staffid"] = $userDetails['id'];
				$_SESSION["name"] = $userDetails['first_name']." ".$userDetails['last_name'];
	...
	public function adminLogin(){		
		...
		if(!empty($_POST["login"]) && $_POST["email"]!=''&& $_POST["password"]!='') {	
			...
			$sqlQuery = "SELECT * FROM ".$this->userTable." 
				WHERE email='".$email."' AND password='".md5($password)."' AND status = 'active' AND (designation='1' OR designation='2' OR designation='3') AND type = 'administrator'"; //1=admin 2=staff 3=teacher 4=student
			$resultSet = mysqli_query($this->dbConnect, $sqlQuery);
			$isValidLogin = mysqli_num_rows($resultSet);	
			if($isValidLogin){
				$userDetails = mysqli_fetch_assoc($resultSet);
	...
	public function editAccount () {
    	...
    	public function staffeditAccount() {
		$message = '';
		$updatePassword = '';
		if(!empty($_POST["passwd"]) && $_POST["passwd"] != '' && $_POST["passwd"] != $_POST["cpasswd"]) {
			$message = "Confirm passwords do not match.";
...
*/
		...
		while( $users = mysqli_fetch_assoc($result) ) {		
			...
			$userRows[] = $users['type'];
			$userRows[] = $users['designation_name'];
			$userRows[] = $status;						
	...
	public function getstaff() {	//formally staffDetails
		$sqlQuery = //"SELECT * FROM ".$this->userTable." 
			//WHERE id ='".$_SESSION["staffid"]."'";
			"SELECT * FROM ".$this->userTable." as u left join ".$this->designationTable." as ds on ds.designation_id=u.designation LEFT JOIN ".$this->staff_infoTable." as si ON si.staff_id=u.id WHERE u.id ='".$_SESSION["staffid"]."'"; 
		$result = mysqli_query($this->dbConnect, $sqlQuery);	
		$userDetails = mysqli_fetch_assoc($result);
		return $userDetails;
	}
